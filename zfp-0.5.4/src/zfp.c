...
{
  return type_precision(field->_type);
}
...
{
  uint64 dims;
  field->_type = (zfp_type)((meta & 0x3u) + 1); meta >>= 2;
...
zfp_stream_open(bitstream* stream)
{
  ...
  if (zfp) {
    ...
    zfp->_exec.policy = zfp_exec_serial;
  }
  return zfp;
}
...
zfp_stream_maximum_size(const zfp_stream* zfp, const zfp_field* field)
{
  ...
  maxbits += values - 1 + values * MIN(zfp->maxprec, type_precision(field->_type));
...
{
  return zfp->_exec.policy;
}

uint
zfp_stream_omp_threads(const zfp_stream* zfp)
{
  return zfp->_exec.params.omp.threads;
}

uint
zfp_stream_omp_chunk_size(const zfp_stream* zfp)
{
  return zfp->_exec.params.omp.chunk_size;
}
...
zfp_stream_set_execution(zfp_stream* zfp, zfp_exec_policy policy)
{
  switch (policy) {
    ...
    case zfp_exec_omp:
#ifdef _OPENMP
      if (zfp->_exec.policy != policy) {
        zfp->_exec.params.omp.threads = 0;
        zfp->_exec.params.omp.chunk_size = 0;
      }
      break;
#else
      return 0;
...
zfp_stream_set_omp_threads(zfp_stream* zfp, uint threads)
{
  if (!zfp_stream_set_execution(zfp, zfp_exec_omp))
    return 0;
  zfp->_exec.params.omp.threads = threads;
  return 1;
}
...
zfp_stream_set_omp_chunk_size(zfp_stream* zfp, uint chunk_size)
{
  if (!zfp_stream_set_execution(zfp, zfp_exec_omp))
    return 0;
  zfp->_exec.params.omp.chunk_size = chunk_size;
  return 1;
}
...
zfp_compress(zfp_stream* zfp, const zfp_field* field)
{
  ...
  uint exec = zfp->_exec.policy;
  uint strided = zfp_field_stride(field, NULL);
  uint dims = zfp_field_dimensionality(field);
  uint _type = field->_type;
  ...
  void (*compress)(zfp_stream*, const zfp_field*) = ftable[exec][strided][dims - 1][_type - zfp_type_int32];
  if (!compress)
    return 0;
...
zfp_decompress(zfp_stream* zfp, zfp_field* field)
{
  ...
  void (*decompress)(zfp_stream*, zfp_field*) = ftable[exec][strided][dims - 1][_type - zfp_type_int32];
  if (!decompress)
    return 0;
